...
/// outside the schema contract (see [`gum_ledger::DbInvoiceError`]).
///
...
fn enrich_response(
    ...
    transfers: Vec<gum_ledger::DbInvoiceTransfer>,
    freshness: Vec<gum_ledger::DbIndexerFreshness>,
) -> Result<DepositRequestResponse, ApiError> {
    let expires_in = row
        .expiration_intent
        .strip_prefix("in:")
...
pub async fn create_deposit_request(
    ...
    // wallet — the deployment's `GUM_RECOVERY_ADDRESS` — whatever add-ons
...
pub(crate) async fn resolve_deposit_request(
    ...
    if let Some(uuid) = gum_core::deposit_request_id(reference) {
        return state
            .repo
...
fn payer_notification_email(payer: &Party, verification: &PayerVerification) -> Option<String> {
    ...
    if email.is_empty() || verification.merchant_auth.is_some() {
        return None;
    }
    Some(email.to_owned())
}
