...
mod tests {
    ...
    fn test_chain_with(
        chain_id: u64,
        tokens: Vec<(gum_core::Currency, Address)>,
        factory: Address,
    ) -> gum_core::ChainConfig {
        gum_core::ChainConfig {
            chain_id,
            tokens: tokens
                .into_iter()
                .map(|(currency, address)| gum_core::TokenConfig { currency, address })
                .collect(),
            factory,
    ...
    fn test_networks(factory: Address) -> Arc<gum_core::ChainRegistry> {
        test_networks_on(vec![
            test_chain_with(
                1,
                vec![
                    (gum_core::Currency::Usdc, Address::ZERO),
                    (gum_core::Currency::Usdt, TEST_USDT),
                ],
                factory,
            ),
    ...
    fn test_networks_with_relay_origin(factory: Address) -> Arc<gum_core::ChainRegistry> {
        test_networks_on(vec![
    ...
    async fn build_on(
        ...
        let (state, storage, chain) = test_state(
            ...
            payer_verification,
            pregenerated_wallets,
            Some(Arc::new(FakeRelay::default()) as Arc<dyn gum_relay::RelayApi>),
        );
        TestApp {
            router: router(state),
            storage,
    ...
    fn sign_challenge(challenge: &Value, key: &[u8; 32]) -> String {
        let typed: gum_core::PayerAttestationTypedData =
            serde_json::from_value(challenge["typed_data"].clone()).unwrap();
        let message = gum_core::PayerAttestation::new(
        ...
        gum_core::sign_payer_attestation(
            key,
            &message,
    ...
    async fn a_payer_may_pay_from_another_chain_through_relay(pool: PgPool) {
        ...
        let intent = gum_ledger::RelayIntentRepository::new(pool.clone())
        ...
        gum_ledger::RelayIntentRepository::new(pool.clone())
    ...
    async fn payer_wallet_binding_derives_the_address_and_is_final(pool: PgPool) {
        ...
        let other = gum_core::wallet_of(&[9u8; 32]);
        let taken = app
            .clone()
            .oneshot(payer_post(
                &challenge_path,
    ...
    async fn a_merchant_session_opens_the_checkout_once_and_carries_the_payer_reference_to_settlement(
        ...
        .bind([1u8; 32].as_slice())
        .bind([3u8; 32].as_slice())
        .bind(gum_core::wallet_of(&PAYER_KEY).as_slice())
        .bind(address.as_slice())
        .bind(uuid)
        .execute(&pool)
        .await
    ...
    async fn preview_session_is_scoped_to_the_owning_merchant_and_a_payable_request(pool: PgPool) {
        ...
        let uuid = gum_core::deposit_request_id(&id).unwrap();
        sqlx::query("UPDATE invoices SET status = 'expired' WHERE id = $1")
            .bind(uuid)
            .execute(&pool)
            .await
    ...
    fn sign_leg(leg: &Value, secret: &[u8; 32]) -> Value {
        let typed: gum_core::AuthorizationTypedData =
        ...
        let kind = match typed.primary_type.as_str() {
            "ReceiveWithAuthorization" => gum_core::AuthorizationKind::Receive,
            _ => gum_core::AuthorizationKind::Transfer,
        };
        let authorization = gum_core::WithdrawalAuthorization {
            kind,
        ...
        let domain = gum_core::TokenDomain {
        ...
        json!({
            "leg_id": leg["id"],
            "signature": gum_core::sign_authorization(secret, &authorization, &domain),
        })
    }
    ...
    async fn withdrawal_is_prepared_signed_submitted_and_polled(pool: PgPool) {
        ...
        let expected_nonce = gum_core::bridge_nonce(
            6,
