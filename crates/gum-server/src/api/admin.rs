...
use gum_core::{DepositRequestResponse, Invoice};
use gum_ledger::ReleasePaymentError;
use uuid::Uuid;
...
pub async fn release(
    ...
    let row = state
        .repo
        .release_attention(id)
        .await
        .map_err(|error| match error {
            ReleasePaymentError::NotFound => ApiError::deposit_request_not_found(),
            ReleasePaymentError::NotAttention => ApiError::deposit_request_not_blocked(),
