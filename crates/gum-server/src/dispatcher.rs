...
fn superseded(invoice: &Invoice, now: DateTime<Utc>) -> Option<&'static str> {
    ...
    if invoice.status != gum_core::InvoiceStatus::Created {
...
async fn deliver_merchant(
    ...
    let currency = event
        .currency
        .parse::<gum_core::Currency>()
        .unwrap_or(gum_core::Currency::Usdc);
...
pub fn guidance(code: &str, currency: gum_core::Currency) -> (String, &'static str) {
