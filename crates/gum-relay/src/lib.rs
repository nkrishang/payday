...
//! deposit request anyway. `gum-server` asks Relay for an `EXACT_OUTPUT`
...
//! seconds. `gum-indexer` then follows the request until it is filled
...
/// `gum-server` quotes; `gum-indexer` follows.
#[async_trait]
