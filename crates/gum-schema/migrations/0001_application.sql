-- Gum application schema baseline (`public`), owned by gum-server.
--
-- This is the whole application schema in one migration. Gum is pre-users,
-- so the schema is edited in place here rather than through incremental
-- migrations; once a real deposit has been accepted the baseline is frozen
-- and a change is a new numbered migration, reviewed for compatibility with
-- the image still running while the new one starts.
--
-- Two more schemas live in the same database and are created by the
-- migrations that follow this one: `bus` (durable messaging between the
-- services, 0002) and `execution` (transaction execution state owned by
-- gum-signers, 0003). gum-server is the only process that writes `public`;
-- gum-signers is the only process that writes `execution`; the indexer
...
-- vocabulary.
--
-- Migrations are applied by `gum-server migrate` (and by the local runner /
-- test harness), never by a service at startup.
...
-- constraint by name (gum_ledger::ISSUER_NAME_UNIQUE).
...
-- ---------------------------------------------------------------------------
-- Sweep jobs: one row per `SweepBatch` command handed to gum-signers. The
-- ledger owns membership (which requests the job carries) and the outcome
-- the signers reported; the transactions themselves live in the `execution`
-- schema, owned by gum-signers. `resolution` is written exactly once, in the
-- transaction that acknowledges the signers' finalized/abandoned event, so
-- a redelivered event finds the job closed and does nothing.
-- ---------------------------------------------------------------------------
CREATE TABLE sweep_jobs (
    ...
    created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
    -- The newest helper transaction the signers reported for the job.
    ...
    CONSTRAINT sweep_jobs_submitted_tx_hash_length
        CHECK (submitted_tx_hash IS NULL OR octet_length(submitted_tx_hash) = 32),
    CONSTRAINT sweep_jobs_resolution_complete CHECK ((resolution IS NULL) = (resolved_at IS NULL)),
    CONSTRAINT sweep_jobs_resolution_valid
        CHECK (resolution IS NULL OR resolution IN ('finalized', 'abandoned', 'rejected'))
);

CREATE INDEX sweep_jobs_open ON sweep_jobs (chain_id, created_at) WHERE resolved_at IS NULL;
...
--
-- Internal statuses: created, funded, fulfilled, recovered, expired. Sweep
-- membership (`sweep_job_id`) and manual attention (`attention_reason`) are
-- orthogonal to the status: a funded request in a sweep job stays `funded`,
-- and a request an operator must look at keeps the lifecycle status it had.
-- `webhook_public_status` maps the pair to the public vocabulary.
-- ---------------------------------------------------------------------------
CREATE TABLE invoices (
    ...
    -- Set when automation has stopped for this request and an operator must
    -- act (`payment_address_blacklisted`, `retries_exhausted`, ...). Cleared
    -- by the release endpoint.
    attention_reason TEXT,

    created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
    updated_at TIMESTAMPTZ NOT NULL DEFAULT now(),

    -- Sweep state: the in-flight job, the block at which the address was
    -- last fully drained, and how many nonzero observations still sit there.
    sweep_job_id UUID REFERENCES sweep_jobs(id),
    ...
    CONSTRAINT invoices_status_valid
        CHECK (status IN ('created', 'funded', 'fulfilled', 'recovered', 'expired')),
...
CREATE INDEX invoices_in_flight ON invoices (sweep_job_id) WHERE sweep_job_id IS NOT NULL;
...
-- for the last successfully indexed range. Both are written by gum-server on
-- the indexer's behalf: the indexer reports, the ledger records.
-- ---------------------------------------------------------------------------
...
-- A chain the indexer cannot trust (a finalized block changed hash). While
-- a fault is open the ledger schedules no new sweep jobs or withdrawal
-- steps on the chain and the signers are told to stop signing for it.
-- Clearing is an operator action (`gum-server chain resume`).
CREATE TABLE chain_faults (
    id UUID PRIMARY KEY,
    chain_id BIGINT NOT NULL,
...
CREATE UNIQUE INDEX chain_faults_open ON chain_faults (chain_id) WHERE cleared_at IS NULL;

-- Which requests a sweep job carries, and what the signers proved about
-- each. Written with the job; `outcome` filled when the job resolves.
CREATE TABLE sweep_job_items (
    job_id UUID NOT NULL REFERENCES sweep_jobs(id),
...
CREATE INDEX sweep_job_items_by_invoice ON sweep_job_items (invoice_id);
...
-- request anyway: gum-server asks Relay for an exact-output quote whose
...
-- and gum-signers relays it: a same-chain `transferWithAuthorization`, or
...
CREATE TABLE withdrawal_legs (
    ...
    -- the execution job id gum-signers reports the step's result under.
    -- Cleared when the step resolves. The transaction itself lives in the
    -- `execution` schema.
    step_job_id UUID,
    ...
    CONSTRAINT withdrawal_legs_step_complete CHECK ((step_job_id IS NULL) = (step_chain_id IS NULL)),
    CONSTRAINT withdrawal_legs_step_only_while_relaying
        CHECK (step_job_id IS NULL OR state IN ('relaying', 'minting')),
    CONSTRAINT withdrawal_legs_hash_lengths CHECK (
        (transfer_tx_hash IS NULL OR octet_length(transfer_tx_hash) = 32)
...
CREATE UNIQUE INDEX withdrawal_legs_step_job ON withdrawal_legs (step_job_id) WHERE step_job_id IS NOT NULL;
CREATE INDEX withdrawal_legs_steps_open ON withdrawal_legs (step_chain_id) WHERE step_job_id IS NOT NULL;
...
-- transfer per account. tx_hash starts NULL and is filled in once the demo
-- payer wallet (held by gum-server: a low-value key that only ever pays
-- these demo deposits) has sent the transfer.
-- ---------------------------------------------------------------------------
...
-- API's.
CREATE FUNCTION webhook_public_status(status TEXT, confirmed_received TEXT, attention_reason TEXT)
RETURNS TEXT LANGUAGE SQL IMMUTABLE PARALLEL SAFE AS $$
    SELECT CASE
        WHEN attention_reason IS NOT NULL THEN 'needs_attention'
        WHEN status = 'created' AND confirmed_received = '0' THEN 'awaiting_deposit'
        WHEN status = 'created' THEN 'partially_deposited'
        WHEN status = 'funded' THEN 'deposited'
...
BEGIN
  ...
  attention := NEW.attention_reason IS NOT NULL AND OLD.attention_reason IS NULL;
