//! Chain adapter: Gum's view of the blockchain.
//!
//! [`ChainReader`] is the read-only seam (headers, logs, receipts, view calls)
//! that gum-indexer and every reconciliation loop depend on. [`ChainExecutor`]
//! adds signing and broadcast on top of it and is only implemented with keys
//! inside gum-signers. Both are traits so the domain logic can be unit-tested
//! against [`mock::MockChain`] without a live RPC. [`AlloyChainClient`] is the
//! production implementation backed by an Alloy HTTP provider, optionally
//! carrying a wallet (the KMS-backed pool keys that call
//! `BatchSweeper.executeBatch` and relay withdrawal steps).
//!
...
pub use deployment::verify_deployment;
...
pub enum ChainError {
    ...
    /// A signing operation was requested from a client connected without a
    /// wallet. Programming error, never retryable.
    #[error("chain client holds no signing keys")]
    ReadOnly,
}
...
fn recipient_restriction_matters(currency: gum_core::Currency) -> bool {
    matches!(currency, gum_core::Currency::Usdc)
}
...
/// Read-only chain access: everything the indexer and reconciliation loops
/// need. Standard Ethereum JSON-RPC methods keep the implementation
/// compatible with QuickNode and local Anvil. A [`ChainReader`] holds no keys.
#[async_trait]
...
/// The newest block whose contents this deployment treats as irreversible
/// on a chain: the node's `finalized` tag or `latest`, per the chain's
/// configured [`gum_core::FinalitySource`], minus its confirmation margin. Every
/// service that concludes anything from chain state (the indexer crediting
/// a payment, the signers concluding a receipt) reads at or below this
/// boundary and verifies the block hash it read against, so the two agree
/// on what "final" means for the same chain.
pub async fn finality_boundary(
    chain: &dyn ChainReader,
    source: gum_core::FinalitySource,
    confirmations: u64,
) -> Result<BlockHeader, ChainError> {
    let anchor = match source {
        gum_core::FinalitySource::Finalized => chain.finalized_header().await?,
        gum_core::FinalitySource::Latest => {
            let latest = chain.latest_block_number().await?;
            return chain
                .block_header(latest.saturating_sub(confirmations))
                .await;
        }
    };
    if confirmations == 0 {
    ...
    chain
        .block_header(anchor.number.saturating_sub(confirmations))
        .await
}

/// Signing and broadcast on top of [`ChainReader`]: only gum-signers holds
/// an implementation with keys. Each pool signer keeps one helper
...
/// Production chain client backed by an Alloy HTTP provider. Built with
/// [`AlloyChainClient::connect`] it is read-only; built with
/// [`AlloyChainClient::connect_signing`] it also holds every pool signer's key
/// for the helper transactions.
pub struct AlloyChainClient {
    provider: DynProvider,
    /// `None` for read-only clients; the executor methods then fail with
    /// [`ChainError::ReadOnly`] instead of reaching the node.
    wallet: Option<EthereumWallet>,
    /// The pool, in configuration order; every address is registered in
    /// `wallet`, which picks the key from a request's `from`. Empty when
    /// read-only.
    signers: Vec<Address>,
    chain_id: u64,
    pacer: RpcPacer,
...
impl AlloyChainClient {
    /// Connect read-only to the RPC endpoint at `rpc_url`
    /// (e.g. `http://127.0.0.1:8545`). `rpc_max_rps` paces outgoing calls
    /// (0 disables pacing).
    pub async fn connect(rpc_url: &str, rpc_max_rps: u64) -> Result<Self, ChainError> {
        ...
        let provider = ProviderBuilder::new()
            ...
            .erased();
        Self::finish_connect(provider, None, Vec::new(), pacer).await
    }

    /// Connect to `rpc_url` signing helper transactions with the keys in
    /// `wallet`; `signers` lists their addresses in configuration order (the
    /// wallet's own map is unordered) and must all be registered in the
    /// wallet. Reads work the same as a read-only client; the wallet only
    /// adds send capability.
    pub async fn connect_signing(
        ...
        let provider = ProviderBuilder::new()
            ...
            .map_err(|error| ChainError::rpc("connect", error))?
            .erased();
        Self::finish_connect(provider, Some(wallet), signers, pacer).await
    }
...
#[async_trait]
impl ChainReader for AlloyChainClient {
    async fn get_chain_id(&self) -> Result<u64, ChainError> {
    ...
    async fn probe_failure(
        ...
        let restricted = |account: Address| match currency {
            gum_core::Currency::Usdc => self.optional_bool(
                token,
                isBlacklistedCall { account }.abi_encode().into(),
                block,
            ),
            gum_core::Currency::Usdt => {
...
impl ChainExecutor for AlloyChainClient {
    ...
    async fn prepare_call(
        ...
        let tx = TransactionRequest::default()
            ...
            .with_input(calldata);
        let wallet = self.wallet.as_ref().ok_or(ChainError::ReadOnly)?;
        sign_request(wallet, tx).await
    }
...
mod tests {
    ...
    fn usdt0_blocks_senders_not_recipients() {
        assert!(recipient_restriction_matters(gum_core::Currency::Usdc));
        assert!(!recipient_restriction_matters(gum_core::Currency::Usdt));
    }
