...
fn layout(invoice: &DepositRequestResponse) -> Vec<Line> {
    ...
    lines.field("Deposit address", &invoice.address);
    ...
    lines.text(
        Font::Regular,
        8.0,
        "The deposit address commits to this deposit request: the hash above, a random nonce, and \
         the derived salt reproduce it offline from the Proof of Payment.",
    );
    lines.finish()
}
