...
fn request_pdf() {}
#[utoipa::path(get, path="/v1/deposit-requests/{id}/proof", operation_id="getProofOfPayment", tag="deposit-requests", params(("id"=String, Path)), responses((status=200,description="Verifiable offline; gateway_core::verify_proof holds the checks",body=ProofOfPayment),(status=401,body=ErrorResponse),(status=404,body=ErrorResponse),(status=409,description="deposit_request_not_settled",body=ErrorResponse),(status=429,body=ErrorResponse)), security(("apiKey"=[])))]
fn proof() {}
