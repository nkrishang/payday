 ...
 * token -> { id, emailVerified }. A session is good for exactly the deposit request
 * it was started on.
 */
...
async function payer(req, res, url) {
  const match = url.pathname.match(
    /^\/v1\/payer\/deposit-requests\/([^/]+)(\/qr|\/attachment|\/verify|\/verify\/email\/start|\/verify\/email\/confirm|\/session)?$/,
  );
