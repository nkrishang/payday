...
import { PayerPreview } from "./payer-preview";
 ...
 *
 * The row above already carries what a list is scanned for — who, how much,
 * where it stands. This answers the questions that follow from clicking it,
 * and answers the first two as pictures rather than as fields: how much of the
 * amount has arrived, and how far through its life the request is. The rest is
 * grouped by what a merchant came for — the money, the payer's proof, and the
 * files — with nothing repeated from the row.
 */
...
/** Where the request stands, as the merchant's own milestones. */
type Milestone = { label: string; at: string | null; tone: "done" | "next" | "todo" | "bad" };

function lifecycle(payment: Payment): Milestone[] {
  const funded: Milestone = {
    label: "Funded",
    at: payment.paid_at,
    tone: payment.paid_at ? "done" : "next",
  };
  // The last milestone is whichever way it ended, so a returned request does
  // not sit forever beside a "Settled" step it will never reach.
  const last: Milestone =
    payment.status === "returned"
      ? { label: "Returned", at: payment.expired_at, tone: "bad" }
      : payment.status === "expired"
        ? { label: "Expired", at: payment.expired_at, tone: "bad" }
        : {
            label: "Settled",
            at: payment.settled_at,
            tone: payment.settled_at ? "done" : payment.paid_at ? "next" : "todo",
          };
  return [{ label: "Issued", at: payment.created_at, tone: "done" }, funded, last];
}
    ...
    <div className="grid gap-5 px-4 pt-4 pb-5">
      <div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_300px]">
        <div className="grid min-w-0 gap-5">
          {/* How much has arrived, as a bar: the one number a merchant opens a
              row to see, and the one that reads faster drawn than written. */}
          <div>
            <div className="flex flex-wrap items-baseline justify-between gap-x-4 gap-y-1">
              ...
              {/* Only while it can still be paid: once a request has settled
                  or closed, its deadline is history the line below tells. */}
              {live ? (
                <p className={cn("text-[12px]", overdue ? "text-warning/80" : "text-faint")}>
                  {overdue ? "Deadline passed" : "Expires"}{" "}
                  {formatRelative(payment.expires_at, now)}
                  <span className="text-faint/70"> · {formatDate(payment.expires_at)}</span>
                </p>
              ) : null}
            </div>
            {/* The track carries the role, not the fill: an empty request
                fills nothing, and a zero-width element is no control at all. */}
            <div
              role="progressbar"
          ...
          <Lifecycle steps={lifecycle(payment)} live={live} now={now} />
        ...
        <div className="grid min-w-0 content-start gap-4">
          <PayerPreview id={payment.id} paymentUrl={payment.payment_url} />
...
/** Issued, funded, settled — the request's life as three points on a line. */
function Lifecycle({ steps, live, now }: { steps: Milestone[]; live: boolean; now: number }) {
  return (
    <ol aria-label="Progress" className="flex min-w-0 items-start">
      {steps.map((step, index) => {
        const done = step.tone === "done";
        const bad = step.tone === "bad";
        const next = step.tone === "next" && live;
        return (
          <li
            key={step.label}
            className={cn(
          ...
          >
            <div className="flex items-center">
              <span
                className={cn(
                  ...
                  done && "border-brand-green bg-brand-green text-brand-black",
                  bad && "border-warning text-warning",
                  next && "border-brand-yellow",
                  !done && !bad && !next && "border-line-strong",
                )}
              >
                {done ? <Check className="size-2.5" strokeWidth={3.5} /> : null}
                {next ? <span className="size-1.5 rounded-full bg-brand-yellow" /> : null}
              </span>
              {index < steps.length - 1 ? (
                <span
                  className={cn(
                    "mx-2 h-px flex-1",
                    steps[index + 1]?.tone === "done" ? "bg-brand-green/50" : "bg-line-strong",
                  )}
                />
              ) : null}
            </div>
            <span
              className={cn(
                ...
                done ? "text-ink" : bad ? "text-warning" : next ? "text-ink" : "text-faint",
              )}
            >
              {step.label}
            </span>
            <span className="text-[11.5px] text-faint">
              {step.at ? formatRelative(step.at, now) : "—"}
            </span>
          </li>
        );
      })}
