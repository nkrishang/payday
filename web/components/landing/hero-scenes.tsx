...
import { useInView, useLoopClock, useReducedMotion } from "./stage";

/**
 * The hero's picture of the product: Gum inside a merchant's own app.
 *
 ...
 * user taps Add funds, pays from a wallet in a dialog Gum powers, and
...
export function HeroScenes() {
  ...
  return (
    <div
      ref={frameRef}
      role="img"
      aria-label="Gum inside a merchant's app: the backend creates a deposit request, the user pays from a wallet without leaving the app, and a webhook credits their balance."
      className="relative w-full overflow-hidden rounded-[18px] bg-gum-black text-gum-white"
    ...
    >
      ...
      >
        {/* A quiet pink glow behind the stage: the brand's colour, faded almost to nothing. */}
        <div
          aria-hidden="true"
          className="pointer-events-none absolute top-[-160px] right-[-140px] size-[460px] rounded-full bg-gum-pink/[0.12] blur-[90px]"
        />
      ...
      {CHAPTERS.map((entry, index) => {
        ...
        return (
          <li key={entry.title} className="min-w-0">
            <span className="block h-[2px] w-full overflow-hidden rounded-full bg-gum-white/10">
              <span
                className="block h-full origin-left rounded-full bg-gum-pink"
...
const CREATE_CODE = `const deposit = await gum.depositRequests.create({
  amount: "250.00",
...
const WEBHOOK_CODE = `app.post("/gum/webhook", async (req, res) => {
      ...
      footer={
        responded ? (
          <span className="landing-scene-line flex min-w-0 items-center gap-3">
            <span className="flex shrink-0 items-center gap-2 text-gum-pink">
  ...
  return (
    <Editor
      ...
      footer={
        <>
          {inbound ? (
            <span className="landing-scene-line flex min-w-0 items-center gap-2.5 text-gum-white/85">
              <span className="rounded-[5px] bg-gum-pink px-1.5 py-0.5 text-[11px] font-semibold text-gum-white">
                POST
              </span>
      ...
      {lines.map((line, index) => (
        <span
          ...
          className={cn(
            ...
            running === index && "bg-gum-pink/[0.16]",
            index === CREDIT_LINE && t >= AT.credit && "text-gum-pink",
          )}
        >
...
function Caret() {
  return (
    <span
      aria-hidden="true"
      className="landing-caret ml-px inline-block h-[1.15em] w-[8px] bg-gum-white/80 align-text-bottom"
    />
  );
}
...
function MerchantApp({ t }: { t: number }) {
  ...
  return (
    <div className="relative h-full overflow-hidden rounded-[14px] bg-gum-white text-gum-black shadow-[0_24px_60px_-24px_rgb(0_0_0/0.8)]">
      <div className="flex items-center gap-2.5 border-b border-gum-black/[0.08] px-5 py-3">
        <span className="flex size-6 items-center justify-center rounded-[7px] bg-gum-black text-[12px] font-bold text-gum-white">
          A
        </span>
        <span className="text-[15px] font-semibold tracking-tight">Acme</span>
        <span className="ml-6 text-[13px] text-gum-grey">Dashboard</span>
        <span className="text-[13px] font-medium">Wallet</span>
        <span className="text-[13px] text-gum-grey">Settings</span>
        <span className="ml-auto flex size-7 items-center justify-center rounded-full bg-gum-pink text-[11px] font-semibold text-gum-white">
          JD
        </span>
      </div>
      ...
      <div className="grid grid-cols-[minmax(0,1fr)_minmax(0,1.15fr)] gap-8 px-6 pt-6">
        <div>
          ...
          <div className="mt-6 grid gap-2.5">
            <span
              ...
              className={cn(
                "flex h-11 items-center justify-center gap-2 rounded-[10px] bg-gum-black text-[14px] font-medium text-gum-white transition-transform",
            ...
            <span className="flex h-11 items-center justify-center gap-2 rounded-[10px] border border-gum-black/20 text-[14px] font-medium">
        ...
        <div>
          ...
          <ul className="mt-1 grid text-[14px]">
            {credited ? (
              <li className="landing-scene-line -mx-3 flex items-center gap-3 rounded-[10px] bg-gum-pink/[0.12] px-3 py-2.5">
                <Usdc className="size-8" />
                <span className="min-w-0 flex-1">
                  <span className="block truncate font-medium">Deposit</span>
                  <span className="flex items-center gap-1.5 text-[12px] whitespace-nowrap text-gum-grey">
      ...
      {toast ? (
        <div className="landing-toast absolute bottom-4 left-1/2 flex -translate-x-1/2 items-center gap-2.5 rounded-[12px] bg-gum-black px-4 py-2.5 text-[13.5px] font-medium whitespace-nowrap text-gum-white shadow-[0_12px_30px_-12px_rgb(0_0_0/0.6)]">
...
function Activity({
  ...
  return (
    <li className="flex items-center gap-3 py-3 text-gum-black/80">
      {icon ?? <span className="size-8 rounded-full bg-gum-black/[0.07]" />}
      <span className="min-w-0 flex-1">
        <span className="block truncate">{label}</span>
        <span className="block text-[12px] text-gum-grey">{when}</span>
      </span>
...
/**
 * The add-funds dialog the app opens over its own page, powered by Gum:
...
function DepositDialog({ t, leaving }: { t: number; leaving: boolean }) {
  ...
  return (
    <>
      <div
        className={cn(
          "absolute inset-0 bg-gum-black/35",
          leaving ? "landing-scrim-out" : "landing-scrim-in",
        )}
      />
      <div
        className={cn(
          "absolute top-1/2 left-1/2 w-[400px] -translate-x-1/2 -translate-y-1/2 rounded-[16px] bg-gum-white px-6 pt-5 pb-5 shadow-[0_30px_80px_-24px_rgb(0_0_0/0.5)]",
          leaving ? "landing-dialog-out" : "landing-dialog-in",
        )}
      >
        <div className="flex items-center justify-between">
          <p className="text-[16px] font-semibold tracking-tight">Add funds</p>
          <X className="size-4 text-gum-grey" />
        </div>

        {state === "received" ? (
          <div className="landing-scene-line py-6 text-center">
            <span className="mx-auto flex size-12 items-center justify-center rounded-full bg-gum-pink text-gum-white">
              <Check className="size-6" />
            </span>
            <p className="mt-4 text-[18px] font-semibold tracking-tight">Deposit received</p>
            <p className="mt-2 flex items-center justify-center gap-1.5 text-[13.5px] text-gum-grey">
          ...
          <>
            ...
            <dl className="mt-4 grid gap-2 text-[13.5px]">
              <div className="flex items-center justify-between gap-3">
                <dt className="text-gum-grey">One-time address</dt>
                <dd className="flex items-center gap-2 font-mono">
                  {DEPOSIT_ADDRESS}
                  <Copy className="size-3.5 text-gum-grey" />
                </dd>
              </div>
              <div className="flex items-center justify-between gap-3">
                ...
                <dd className="flex items-center gap-1.5 font-medium">
                  ...
                  Monad <span className="text-gum-grey">·</span>{" "}
            ...
            <span
              ...
              className={cn(
                ...
                state === "ready"
                  ? "bg-gum-black text-gum-white"
                  : "bg-gum-black/[0.08] text-gum-grey",
        ...
        <p className="mt-4 flex items-center justify-center gap-2 text-[12px] text-gum-grey">
          <span className="size-2 rounded-full bg-gum-pink" />
          Secured by Gum
        </p>
      </div>
...
function Token() {
  return (
    <span className="flex items-center gap-1.5 text-[15px] font-medium text-gum-grey">
      <Usdc className="size-5" />
      USDC
    </span>
  );
