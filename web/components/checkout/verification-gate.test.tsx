...
import { lockedDepositRequest, merchantSessionDepositRequest } from "@/test/fixtures";
...
describe("VerificationGate", () => {
  it("shows the issuer, the heading, and the masked mailbox and nothing else", () => {
    const { container } = render(gate(lockedDepositRequest({ heading: "Consulting — August" })));
  ...
  it("tells a merchant-session payer to open it from the app, with nothing to type or click", () => {
    const { container } = render(gate(merchantSessionDepositRequest({ heading: "Deposit 500 USDC" })));
  ...
  it("shows progress while the client secret is being exchanged", () => {
    render(
      gate(merchantSessionDepositRequest(), { ...open, exchangingClientSecret: true }, null, "exchanging"),
    );
  ...
  it("says when the link was already opened, and where to go", () => {
    render(gate(merchantSessionDepositRequest(), open, null, "used"));
  ...
  it("says when the link has expired", () => {
    render(gate(merchantSessionDepositRequest(), open, null, "invalid"));
