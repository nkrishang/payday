...
ACM cannot issue a TLS certificate for `api.gum.money` because the parent
...
The parent domain `gum.money` (hosted at Vercel or another registrar) has CAA
records that only authorize certain CAs (e.g. Let's Encrypt, Sectigo, Google).
ACM needs a CAA record authorizing `amazon.com` within the `api.gum.money`
zone or the parent zone.

## Fix: Add CAA records in Route53

The `api.gum.money` hosted zone is in Route53. Add two CAA records there:
...
{
  "Comment": "Allow Amazon to issue certificates for api.gum.money",
  "Changes": [
    {
      "Action": "UPSERT",
      "ResourceRecordSet": {
...
If you control the parent `gum.money` DNS, you can add the Amazon CAA records
there instead. At Vercel (or your registrar's DNS panel), add:

```
api.gum.money. CAA 0 issue "amazon.com"
api.gum.money. CAA 0 issuewild "amazon.com"
```
