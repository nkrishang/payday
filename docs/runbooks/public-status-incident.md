# Public status incident: payment progression delay
...
`ALARM`.
The status page automatically reports **Payment indexing and settlement** as
...
2. Within 15 minutes, publish a customer incident in the team's approved public
   incident channel: “Payment detection and settlement are delayed. Funds remain
   safe; we are investigating.” Link to the status page and timestamp the update.
3. Triage with the indexer logs and the [stuck invoice](stuck-invoice.md),
...
5. After CloudWatch returns to `OK`, verify the JSON endpoint reports
   `payment_indexing_and_settlement.status` as `operational` for two checks.
