...
sits at the address and is queued for recovery through `Deposit.recover`.
...
requiring the requested amount. Both the expiration timestamp and the recovery
...
recovery wallet by `Deposit.recover`, which anyone may call and which the
