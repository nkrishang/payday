...
Sign in to the dashboard at `payday.sh` with a real mailbox and generate an
API key from its **API key** section (the CLI's own Auth0 login is no longer
accepted by the API). Then give the CLI that key:

```bash
export PAYDAY_API_KEY="<key from the dashboard>"
cargo run --release -p gateway-cli --bin payday -- whoami
