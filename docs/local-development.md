...
recovery term, Payday's dedicated KMS recovery wallet, and after expiry it
...
The recovery wallet is Payday's own dedicated KMS recovery wallet
(`PAYDAY_RECOVERY_ADDRESS`), never a payer-chosen or per-request value:
`gum-server` rejects a create request that carries `refund_address`. Payday
...
Payday's recovery custody (`PAYDAY_RECOVERY_ADDRESS`); there is no flag to
choose it.
...
- `PAYDAY_RECOVERY_ADDRESS` — the Ethereum address of Payday's dedicated KMS
