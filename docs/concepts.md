...
## One invoice, one virtual account, one payer wallet

Every Payday invoice receives a unique EVM address once its payer has attested
the wallet they will pay from. The address is *counterfactual*: Payday
calculates it before deploying the payment contract, so the payer can send USDC
to it as soon as it exists. Until the payer's wallet is bound, the invoice has
...
page, from the wallet they will pay from, over the invoice's attribution hash
and a one-time nonce issued to their session once the invoice's policy is
...
from the invoice and that signature: Payday canonicalizes the issued document
...
settles, but the invoice is flagged `likely_unsolicited_at` and no Proof of
...
withholds the amount, bill-to, notes, reference, PDF, address, URI, and QR
until the payer's session satisfies the policy — only the issuer name and
heading show, with a masked hint of the expected mailbox.

The wallet step follows the policy on every invoice, gated or not: the
...
completed the checks to the wallet that pays: the identity facts are Payday's
word, the wallet signature and the transfers from that wallet are anyone's to
recompute.
...
| `settled` | Exactly the invoice amount reached the payout address at on-time execution; any remainder went back to the payer's wallet. |
...
| More than requested is present at on-time execution | Payout receives exactly the invoice amount; the remainder goes back to the payer's wallet. |
...
payment in the `recovered_funds` ledger and a `payment.recovered_funds` webhook
reports it. Payday does not hold the intended invoice amount either, which
...
A settled invoice can be exported as a Proof of Payment (`payday.proof.v2`):
...
payment, and recovery addresses, the credited USDC transfers, the fulfilment
transaction that executed the payment contract, the attachment's hash, and a
...
the CREATE3 payment address with the wallet as its recovery term, and confirm
that the address received transfers from that wallet covering the invoice
...
The proof establishes invoice-to-wallet-to-address-to-transfer integrity: the
sentence it supports is that a session which satisfied this invoice's policy
proved control of wallet W, every credited transfer came from W to an address
that can only belong to this invoice and this attestation, and exactly the
...
wallet, and nonce, so it belongs to that invoice and that payer alone. Funds
credited from any other wallet make the proof unavailable
(`409 payment_sender_mismatch`): Payday does not issue a proof it cannot
