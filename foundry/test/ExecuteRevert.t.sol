...
contract ExecuteRevertTest is Test {
    ...
    function test_underpayment_reverts_with_DeploymentFailed_and_leaves_no_code() public {
        ...
        address paymentAddress = factory.paymentAddress(address(token), amount, address(0xBEEF), salt);
        token.mint(paymentAddress, amount - 1);

        vm.expectRevert(CREATE3.DeploymentFailed.selector);
        factory.execute(address(token), amount, address(0xBEEF), salt);
    ...
    function test_already_executed_reverts_with_DeploymentFailed() public {
        ...
        address paymentAddress = factory.paymentAddress(address(token), amount, receiver, salt);
        token.mint(paymentAddress, amount);

        factory.execute(address(token), amount, receiver, salt);
