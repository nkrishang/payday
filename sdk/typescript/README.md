...
at creation: the payment address exists only once the payer has attested, from
...
the `payment.ready` webhook, or poll until `address` is set, before quoting an
...
Gated invoices withhold their content from the payer page until verification
completes; every request, gated or not, withholds its payment address until the
