...
secrets. The `recovery` KMS key is the dedicated recovery wallet: its derived
Ethereum address is passed to the API task as `PAYDAY_RECOVERY_ADDRESS` and is
the recovery term committed into every deposit address. Recovered
funds — overpayment remainders, expired balances, late transfers, and manual
recoveries — land in that custody on-chain, and returning them to the payer
is a manual operator procedure that signs with this key after review (see
`docs/production-runbook.md` §"Recovery custody and manual returns"). No
service task role can sign with it. The API task definition's
precondition fails the plan until the address is set.
